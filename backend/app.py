...
async def perform_image_factcheck(image_bytes: bytes, mime_type: str, caption: str = None):
    ...
    prompt_parts = [
        {
            "text": f"""You are an expert fact-checker. You are given an image. Your job is to analyze the text or message in the image and determine, as accurately as possible, if it is factual or contains misinformation,Your answer must be concise and must not exceed 100 words.
            
            {f"Context provided: {caption}" if caption else "No additional context provided."}
            
            Instructions:
            - Your answer must be concise and must not exceed 100 words.Summarize your reasoning if needed to stay within the word limit.
